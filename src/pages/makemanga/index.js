...
const MakeManga = () => {
  ...
  const callGenerateStoryEndpoint = useCallback(async () => {
    ...
    setComicOutput([]);
    try {
      const response = await callApi("/api/sentence", {
        method: "POST",
        headers: {
  ...
  return (
    <div style={{display: 'flex', placeContent: 'center flex-start', alignItems: 'center', backgroundSize: 'center', height: '70vh', overflow: 'auto', position: 'relative',
      flexFlow: 'column nowrap', gap: '10px'
    }}>
      <Head>
        <title>AI Manga Maker</title>
      </Head>
      <div style={{display: 'flex', placeContent: 'center flex-start', alignItems: 'center', flex: '0 0 auto', flexFlow: 'column nowrap', gap: '10px',
      height: 'fit-content', maxWidth: '975px', overflow: 'auto', padding: '50px', position: 'relative', width: 'auto', gap: '64px'
      }}>
        {
          currentStep >= 0 && (
